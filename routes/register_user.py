...
def register_user():
    ...
    # ✅ تحقق من عدم تكرار اسم المستخدم في users أو pending_users
    ...
    # ✅ تحقق من عدد الحسابات لنفس الإيميل (حد أقصى 3 في users + pending_users)
    ...
    if (user_count + pending_count) >= 3:
        return jsonify({'error': 'Maximum 3 accounts allowed per email'}), 400
    ...
    try:
        # ✅ إدخال المستخدم في جدول pending_users
        cursor.execute("""
