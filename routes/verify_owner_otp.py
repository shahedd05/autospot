...
def verify_owner_otp():
    ...
    try:
        ...
        cursor.execute("""
            SELECT id, register_number, national_number, owner_name, email, password_hash
            FROM pending_owners
        ...
        # ✅ تحقق من الـ OTP (ثابت للتجربة)
        if otp != "1234":
            return jsonify({'success': False, 'error': 'Invalid OTP'}), 401

        # ✅ نقل الحساب من pending_owners إلى owners
        cursor.execute("""
        ...
        # ✅ حذف الحساب من pending_owners بعد التحقق
        cursor.execute("DELETE FROM pending_owners WHERE id=%s", (pending_owner['id'],))
        mysql.connection.commit()

        return jsonify({'success': True, 'message': 'OTP verified (1234), account activated'}), 200
